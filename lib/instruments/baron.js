...
const ITEM_BANK = {
  ...
  23: { text: "Me es dificil compartir mis sentimientos mas intimos", reverse: false },
  ...
  43: { text: "Me resulta dificil cambiar de opinion", reverse: false },
  ...
  103: { text: "Me resulta dificil cambiar mis costumbres", reverse: false },
...
function resolvePrimaryComponent(itemId, membershipMap) {
  if (membershipMap.has(itemId)) {
    return membershipMap.get(itemId)[0].componentKey;
  }

  const future = [];
  for (let next = itemId + 1; next <= 133; next += 1) {
    if (membershipMap.has(next)) {
      future.push(membershipMap.get(next)[0].componentKey);
      break;
    }
  }
  ...
  for (let prev = itemId - 1; prev >= 1; prev -= 1) {
    if (membershipMap.has(prev)) {
      return membershipMap.get(prev)[0].componentKey;
    }
  }
